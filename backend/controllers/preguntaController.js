...
exports.getPreguntasParaTest = async (req, res) => {
    ...
    const sql = `
        ...
        LEFT JOIN respuestapreguntas rp ON p.id_pregunta = rp.id_pregunta
...
exports.createPregunta = async (req, res) => {
    ...
    try {
        ...
        for (const resp of respuestas) {
            ...
            const sqlInsertRespuesta = 'INSERT INTO respuestas (texto, es_correcta) VALUES (?, ?)';
            const [respuestaResult] = await connection.query(sqlInsertRespuesta, [resp.texto, resp.es_correcta ? 1 : 0]);
            const id_respuesta = respuestaResult.insertId;

            // Vincular respuesta con pregunta 
            const sqlInsertRpLink = 'INSERT INTO respuestapreguntas (id_pregunta, id_respuesta) VALUES (?, ?)';
            await connection.query(sqlInsertRpLink, [id_pregunta, id_respuesta]);
        }
